...
kubectl run --rm -it --restart=Never --image bjd145/utils:3.7 --env="hostname=${hostname}" --env="keyid=${keyid}" getkeys --command -- /bin/bash -c 'curl --header "Content-Type: application/json" --request GET https://${hostname}/api/keys/${keyid}'
