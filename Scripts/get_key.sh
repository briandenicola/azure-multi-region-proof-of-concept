...
kubectl run --rm -it --restart=Never --image bjd145/utils:2.2 --env="hostname=${hostname}" --env="keyid=${keyid}" getkeys -- /bin/bash -c 'curl --header "Content-Type: application/json" --request GET https://${hostname}/api/keys/${keyid}' | jq
