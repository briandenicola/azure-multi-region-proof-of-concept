...
resource "azurerm_managed_redis_geo_replication" "example" {
  managed_redis_id = module.regional_resources[element(var.locations, 0)].AZURE_REDIS_CLUSTER_DATABASE_ID

  linked_managed_redis_ids = [
    for i in toset(var.locations) : "${module.regional_resources[i].AZURE_REDIS_CLUSTER_DATABASE_ID}" 
  ]
}